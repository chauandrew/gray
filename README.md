...
A Chrome extension that blocks images and mutes video by default, everywhere. Click a gray box to reveal what's underneath.

No account, no server, no tracking. Everything lives in `chrome.storage.local` on your machine.

## Why

Images and autoplaying video are most of the weight (and most of the distraction) on a typical page. Gray blocks them at the network level before they ever download, and mutes video as soon as it starts playing. When you actually want to see something, click it.

## Features

- **Blocks images before they load** via a `declarativeNetRequest` rule, not a CSS filter — the bytes never come down.
- **Click-to-reveal** — click any blocked image to load and show it; click again to re-hide it.
...
- Click any gray box on a page to reveal the image or video underneath; click it again to re-hide it.
...
| Content script on `<all_urls>` | Paint blocked-image boxes, mute video, and handle click-to-reveal on every page. |
