...
2. Zip the contents of `extension/` (not the repo root, not the folder itself — the files inside it) and upload as a new item.
...
```
Blocks images and mutes video by default, everywhere. Click any gray box to reveal it. Nothing leaves your browser.
```

**Detailed description:**
```
Gray blocks images and mutes video by default on every site you visit. Click a gray box to reveal the image underneath; click it again to hide it.

FEATURES
- Blocks images before they download, at the network level — not just hidden with CSS.
- Mutes video automatically.
- Click any blocked image to reveal it, click again to re-hide it.
...
```
Gray blocks images and mutes video by default on web pages, letting users reveal them on click, with per-site exceptions.
```
...
- Host permission / content script on all URLs — "Blocking and mute are default-on across every site, so the content script needs to run everywhere to paint placeholders, mute video, and handle click-to-reveal."
